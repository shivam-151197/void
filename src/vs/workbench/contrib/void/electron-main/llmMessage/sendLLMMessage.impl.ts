...
const _embeddingModelName = ({ settingsOfProvider, providerName, modelName }: Pick<EmbeddingsParams, 'settingsOfProvider' | 'providerName' | 'modelName'>): string => {
	const providerModels = settingsOfProvider[providerName]?.models ?? [];
	const embeddingModel = providerModels.find(m => !m.isHidden && /embed|bge|e5|nomic/i.test(m.modelName));
	return embeddingModel?.modelName ?? modelName ?? 'text-embedding-3-small';
}
const _sendOpenAICompatibleEmbeddings = async ({ text, onSuccess, onError, settingsOfProvider, providerName, modelName }: EmbeddingsParams) => {
	try {
		const openai = await newOpenAICompatibleSDK({ providerName, settingsOfProvider })
		openai.embeddings.create({
			model: _embeddingModelName({ settingsOfProvider, providerName, modelName }),
			input: text,
		})
			.then(response => {
