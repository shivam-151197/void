...
const builtinToolNameToComponent: { [T in BuiltinToolName]: { resultWrapper: ResultWrapper<T>, } } = {
	...
	'semantic_search': {
		resultWrapper: ({ toolMessage }) => {
			...
			if (toolMessage.type === 'success') {
				const { result, rawParams } = toolMessage
				componentParams.numResults = result.symbols?.length ?? 0
				componentParams.children = undefined // TODO: render symbols
			}
			else if (toolMessage.type === 'tool_error') {
				const { result } = toolMessage
				componentParams.bottomChildren = <BottomChildren title='Error'>
...
Execute ALL remaining pending tasks from the latest approved <plan> continuously.

Rules:
- Execute one tool call per response. After each tool result, you will be called again — immediately continue to the next step.
- After completing each task, output a <task_summary> then proceed to the next task without stopping.
- If a step is already done (e.g. branch exists, file already created), skip it.
- Do NOT stop until all tasks are complete or you encounter an error that requires user input.
- After the final task, output a <walkthrough>.`
