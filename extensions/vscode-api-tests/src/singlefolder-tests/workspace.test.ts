...
suite('vscode API - workspace', () => {
	...
	test('openTextDocument, actual casing first', async function () {
		...
		fs.writeFile(uriOne, Buffer.from('one') as any, { create: true, overwrite: true });
		fs.writeFile(uriTwo, Buffer.from('two') as any, { create: true, overwrite: true });
	...
	test('Support creating binary files in a WorkspaceEdit', async function (): Promise<any> {
		...
		ws.createFile(fileUri, { contents: data as any, ignoreIfExists: false, overwrite: false });
	...
	test('encoding: decode', async function () {
		...
		// without setting
		assert.strictEqual(await vscode.workspace.decode(Buffer.from('Hello World') as any, uri), 'Hello World');
		assert.strictEqual(await vscode.workspace.decode(Buffer.from('Hellö Wörld') as any, uri), 'Hellö Wörld');
		...
		assert.strictEqual(await vscode.workspace.decode(Buffer.from('Hello World') as any, uri, { encoding: 'foobar123' }), 'Hello World');
